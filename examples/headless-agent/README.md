...
[`comfy.cordis.yml`](comfy.cordis.yml) adds the ComfyUI capability (`ctx.comfy`) and its model-facing tools to the advanced composition, pointing `baseUrl` at the LAN comfy-api-proxy (port 8189) and `serverUrl` at the ComfyUI server (port 8188). Both endpoints are unauthenticated on this LAN, so no credentials are needed; the model discovers models and node schemas with the `comfy_*` tools, submits workflows, and polls jobs to completion. Outputs land in the server's output directory and as asset URLs in job results.
