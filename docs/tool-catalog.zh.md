...
请求取消一个 ComfyUI 作业。幂等；返回作业的当前状态。
...
获取 ComfyUI 作业的状态、进度、输出与错误；可选择阻塞到终态。
...
获取单个 ComfyUI 节点类的输入模式：输入名、类型、默认值与允许值。
...
列出 ComfyUI 模型目录（省略 folder 时）或单目录内的模型文件。
...
提交 API 格式的 ComfyUI 工作流图执行，并返回排队中的作业 id。
...
将本地文件上传到 ComfyUI 服务器作为工作流输入资产，并返回其资产 id。
